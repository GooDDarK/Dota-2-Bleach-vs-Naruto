...
function ConjureImage( event )
 ...
 local illusion = CreateUnitByName("zabuza_bunshin", origin, true, caster, nil, caster:GetTeamNumber())
 PrintTable(illusion)
 ...
 if caster:HasModifier("modifier_kisame_metamorphosis") then 
    illusion:SetOriginalModel("models/zabuza/zabuza.vmdl")
 end
...
-- add water prison (channeled hold) to bunshin
AbilityWater = illusion:FindAbilityByName("zabuza_executioners_blade")
...
illusion:SetBaseDamageMin(caster:GetAverageTrueAttackDamage(nil) / 100 * damage_percentage)
illusion:SetBaseDamageMax(caster:GetAverageTrueAttackDamage(nil) / 100 * damage_percentage)
